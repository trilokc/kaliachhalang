...
public class JdbcOwnerRepositoryImpl implements OwnerRepository {
  ...
  //mine
    public List<Owner> getOwners() throws DataAccessException {
    	List<Owner> owners = getAllOwners();      // getOwnersFromDatabase 
    	loadOwnersPetsAndVisits(owners); 
    	return manipulate(owners);
    }
    
   private List<Owner> manipulate(List<Owner> owners) {
    	int maxpets = 0; 
    	Owner owner = null; 
    	List<Owner> result = new ArrayList(); 
	   for(int i = 0; i < owners.size(); i++) { 
		   Owner o = owners.get(i); 
		  int size = o.getPets().size(); 
		  if(maxpets < size)  { 
			  maxpets = size; 
			  owner = o; 
   ...
   List<Owner> getAllOwners() {
	   Map<String,Owner> m = new HashMap<String,Owner>();
   	//m.put(key, value);
   	List<Owner> owners = this.namedParameterJdbcTemplate.query("Select * from Owners",
   ...
   public List<Owner> ownersByPetType(String petId) throws DataAccessException {
	   return ownerByPetTypeManipulation(petId); 
   }
  List<Owner> ownerByPetTypeManipulation(String petId) {
	  
	 // getDataFromDB();
	 // processAlgoForPetType();
	 // convertIntoListOfOwner(); 
	  return null; 
  }
    ...
    public void save(Owner owner) throws DataAccessException {
        ...
        if (owner.isNew()) {
            ...
            owner.getFirstName().toUpperCase();
            owner.getLastName().toUpperCase();
        } else {
