...
public class JdbcVetRepositoryImpl implements VetRepository {
   ...
   // Starting point for logic algorithm
   String getVetBdone() { 
	   
	  List<Integer> recs = getDbrecords(3);
	 //Integer i = recs.get(1); 
	   Date d = findRelatedThings(recs.get(0));
	   Integer i =  finalProcess(d); 
	   return i.toString();
	   
   }
   
   List<Integer> getDbrecords(int id) { 
	   return null; 
   }
   
   Date findRelatedThings(Integer i) { 
	   return null; 
   }
   
   Integer finalProcess(Date d) { 
	   return null; 
   }
}
